...
public class Inheritance {
	...
	public static void main(String[] args) {
		 ...
		 * - 두개 이상의 클래스를 만드는데 곧통된 멤버가 있는 경우 부모 클래스로 만든다.
		 * 
		 * << super >>
		 ...
		 * - 부모 타입으로 생성된 객체는 부모타입에 정의된 멤버만 사용 가능하다.
		 * - 부모 타입과 자식타입은 서로 형변환이 가능하다.
		 ...
		 *           
		 *           
		 * - 부모와 자식에 동일한 메서드가 있는 경우 (-> 오버라이딩을 했다) 자식의 메서드를 사용한다.
		 * - 부모와 지식에 동일한 멤버변수가 있는 경우 부모타입에서는 부모의 변수를 
		 ...
		 * - default : 같은 패키지에서만 접근이 가능하다. (생략했을때)
...
//		am.privateMethod();
		...
		System.out.println(new Inheritance()); //f_oop2.Inheritance@1a3a2a52
		System.out.println(new Inheritance()); //f_oop2.Inheritance@60b07af1
		System.out.println(Singleton.getInstance()); //f_oop2.Singleton@625795ce
		System.out.println(Singleton.getInstance()); //f_oop2.Singleton@625795ce
