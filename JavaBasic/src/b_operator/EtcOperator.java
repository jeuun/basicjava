...
public class EtcOperator {
	...
	public static void main(String[] args) {
		/* 
		 * << 비트 연산자>> - 비트단위로 연산, byte는 8개의 비트로 구성, 1byte = 01(2진수)8자리
		 ...
		 *  - >>(쉬프트)	:피연잔사의 각 자리를 오른쪽으로 이동시킨다. 빈칸은 앞자리와 동일한 값으로 채운다.
		 *  
		 *  << 기타 연산자>>
		 *  -.(참조연산자)	: 특정 범위 내에 속해 있는 멤버를 지칭할 때 사용한다.
		 *  - (type)		: 형변환(casting)
		 *  - ?:(삼항연산자)	: 조건식 ? 조건식이 참일 경우 수행할 문장 : 조건식이 거짓일 경우 (3번째) 수행 할 문장 -중요
		 *  - instanceof	: 참조형타입 확인
		 */
		...
		/*
		 * - 10 진수 -> 2진수 : 10진수 값을 1이 될때까지 2로 나눈다.
		 * 10    /2
		 * 5  0
		 * 2  1
		 * 1  0
		...
		//<< : 00010100
		System.out.println(10 << 1); //20 - 1 한칸만큼 이동, 뒤에 오는 숫자는 이동하는 숫자
		//>> : 00000101
		...
		int x = 10;
		int y = 20;
		int result = (x < y) ? x : y; // 조건식의 결과에 따라 두번째 문장 세번째 문장이 실행될 수 있다.
		...
		int s =3;
		String gen = s == 1 || s == 3 ? "남자" : s == 2 || s ==4 ? "여자" : "잘못 입력하셨습니다.";				
