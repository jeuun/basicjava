...
//ctrl + Shift + o
import java.util.Scanner; //클래스의 위치
...
public class variable {
	...
	public static void main(String[] args) {
		 ...
		 * - [여러단어로 이루어진 경우 단어의 첫 글자를 대문자로 한다.](상수는 언더바로 구분)
		 * - [클래스명의 첫글자는 대문자로 한다.]
		 * - [패키지명은 모두 소문자로 한다.]
		 */
		...
		byte _byte; short _short; int _int; long _long;
		float _float; double _double; char _char;
		boolean _boolean;
		...
		//기본값 : 전역변수(매서드 밖에 있는 변수)에 아무것도 저장하지 않았을때 저장되어 있는 값
		_byte = 0;
		_short = 0;
		_int = 0;
		_long = 0L;
		...
		//표현범위가 큰 타입에서 작은 타입으로 형 변환 할 경우
		//데이터가 손실될 수 있다.
		...
		//표현범위가 작은 타입에서 큰 타입으로 형 변환 할 경우
		//데이터가 손실될 염려가 없다.
		//그러므로 형변환을 생략할 수 있다.
		_int = _byte;
		_long = _int;
...
//		System.out.println("문자열을 입력해주세요>");
//		String input1 = s.nextLine();
//		System.out.println("입력받은 문자열을 출력해주세요 : " + input1);
//			
//		//숫자를 입력받고 입력받은 숫자를 출력해주세요
//		System.out.println("숫자를 입력해주세요>");
//		int input2 = Integer.parseInt(s.nextLine());
//		System.out.println("입력받은 숫자를 출력 : " + input2);	
//		
